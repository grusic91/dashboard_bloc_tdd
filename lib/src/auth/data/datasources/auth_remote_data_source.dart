...
class AuthRemoteataSourceImpl implements AuthRemoteDataSource {
  const AuthRemoteataSourceImpl({
  ...
  Future<LocalUserModel> signIn({
    ...
    try {
      ...
      // exists is function from firebase documents
      // when user exist
      if (userData.exists) {
  ...
  Future<void> signUp({
    ...
    try {
      final userCard = await _authClient.createUserWithEmailAndPassword(
        email: email,
        password: password,
      );

      await userCard.user?.updateDisplayName(fullName);
      await userCard.user?.updatePhotoURL(kDefaultAvatar);
    ...
    try {
      switch (action) {
        ...
        case UpdateUserAction.password:
          ...
          final newData = jsonDecode(userData as String) as Map;
